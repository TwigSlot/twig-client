<template>
  <div class="main-container">
    <ControlPanelVue
      ...
      @customkeydown="keydown"
    >
      <template v-slot:panel>
        <DataPanelVue
          @pauseKeyDown="pauseKeyDown"
          @resumeKeyDown="resumeKeyDown"
          :data_panel="dataPanel"
          @updatedDataPanel="updatedDataPanel"
        ></DataPanelVue>
      </template>
    </ControlPanelVue>
  </div>
  ...
  <div id="graph" ref="div_ref">
    ...
    v-model:selected-nodes="graphData.selectedNodes"
    v-model:selected-edges="graphData.selectedEdges"
...
function get_items() {
  const request_url =
    import.meta.env.VITE_API_URL + `/project/${project_id.value}`;
  ...
  methods: {
    ...
    keydown: function (e: any) {
      ...
      if (e.key == "e") {
        ...
        if (
          selected_nodes.value.length > 0 &&
          confirm(`Delete ${selected_nodes.value.length} nodes?`)
        ) {
        ...
        if (
          selected_edges.value.length > 0 &&
          confirm(`Delete ${selected_edges.value.length} edges?`)
        ) {
